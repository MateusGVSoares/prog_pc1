...
void rand_matrix(Matrix * target,int range, int offset)
{
    // Preenche a matriz de tamanho MxN com elementos randômicos 
    for(int i = 0; i < target->rows; i++)
    {
        for(int j = 0; j < target->cols; j++)
            target->matriz[i][j] = offset + (rand()%range);
    }
}
...
int main()
{
    ...
    rand_matrix(target_a,200,100);
    matrix_to_string(*(target_a));
    
    float * ret_pt = get_rows_avg(target_a);
    float ret_val = 0.0;
    print_vector(ret_pt, target_a->rows);
    int find_pos = find_max_val(ret_pt,target_a->rows,&ret_val);
    printf("\n Mes = %d / Valor = %0.4f \n",find_pos + 1,ret_val);
    free_matrix(target_a);
}
